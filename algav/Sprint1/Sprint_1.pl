...
% Heurística para minimização dos tempos de ocupação da máquina
spt(L,LR):-
    get_time(Ti),
    sort_by_type(L, LR),
    get_time(Tf),Tcomp is Tf-Ti,
...
% Heurística para somatório dos tempos de atraso das operações
